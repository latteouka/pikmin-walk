...
- 前端 `flower-cruise.html` plant 模式：**已調校到最佳值、寫死不開放調整**——半徑 **25m**、速度 **~14 km/h**、每朵繞**一圈 ~40s**（送 `dwell_radius_m:25` / `speed_kmh:14` / `dwell_each_s:40`，單朵不送 dwell=無限繞）。強制 loop、隱藏所有滑桿（`PLANT` 常數）。harvest/quick_harvest 仍有速度滑桿。
